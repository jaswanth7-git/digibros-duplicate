<?php
 include('conn.php');
 $sql = "DELETE  from `orders` WHERE pstatus = 'no' ";
 mysqli_query($conn, $sql);
 // to be removed
 
 $sql = "SELECT * FROM orders WHERE pstatus = 'yes' and shipstatus = 'no'";
                  ...
                  <tbody>
                    <?php 
                     while($row = $result->fetch_assoc()) {
                      ...
                      <td>$row[pquantity]</td>
                      <td>Rs $row[tprice]</td>
                      <td><a href='aaccept.php?oid=$row[orderid]' type='button' class='btn btn-success' style='margin-right: 15px; color: white;'>Accept</a><a href='adelete.php?oid=$row[orderid]' type='button' class='btn btn-danger' style='color: white;'>Decline</a></td>
                     </tr>
